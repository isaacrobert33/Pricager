...
BASE_URL = "https://topshelfaquatics.com/?s={}&post_type=product&dgwt_wcas=1&orderby=price-desc"
headers = {
    'dnt': '1',
...
def search_topshelf(queries: list, export=False):
    ...
    for url in urls:
        ...
        product_data = list()
        try:
            product_list = soup.find_all("ul", class_="products")[0].find_all("li", class_="product")
        except:
        ...
        for product in product_list:
            try:
                try:
                    product_title  = product.find_all("h2", class_="woocommerce-loop-product__title")[0].text.strip()
...
print(search_topshelf(queries=queries, export=True))